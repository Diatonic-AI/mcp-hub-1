...
export class MCPConnection extends EventEmitter {
  constructor(name, config, marketplace, hubServerUrl) {
    ...
    if (this.transportType === 'stdio' && config.dev) {
      ...
      this.devWatcher.on('filesChanged', (data) => this._handleDevFilesChanged(data));
    }
  }
  ...
  async connect(config) {
    try {
      ...
      // Optional: if python sidecar execution prefix is configured, wrap python/uv commands
      try {
        const sidecarPrefix = process.env.PYTHON_SIDECAR_EXEC && process.env.PYTHON_SIDECAR_EXEC.trim();
        if (sidecarPrefix && ['uvx','uv','python','python3'].includes(resolvedConfig.command)) {
          // Transform command into bash -lc "<prefix> <command> <args...>"
          const originalCmd = resolvedConfig.command;
            const originalArgs = Array.isArray(resolvedConfig.args) ? resolvedConfig.args.join(' ') : '';
          const composite = `${sidecarPrefix} ${originalCmd} ${originalArgs}`.trim();
          resolvedConfig.command = 'bash';
          resolvedConfig.args = ['-lc', composite];
          logger.debug(`Sidecar wrapping applied for '${this.name}': ${composite}`);
        }
      } catch (wrapErr) {
        logger.warn(`Failed applying sidecar wrapping for '${this.name}': ${wrapErr.message}`);
      }
  ...
  */
  ...
  registerActivity() {
    // Reset idle timer on any activity if connected
    if (this.status === ConnectionStatus.CONNECTED) {
      this._resetIdleTimer();
    }
  }
  ...
  _resetIdleTimer() {
    ...
    if (!this.idleTimeoutMs || this.status !== ConnectionStatus.CONNECTED) return;
    this._idleTimer = setTimeout(() => {
      // Best-effort idle disconnect
      this.disconnect('idle-timeout').catch(() => {});
    }, this.idleTimeoutMs);
  }
  ...
  async resetState(error) {
    ...
    // Clear idle timer and settings
    if (this._idleTimer) {
      clearTimeout(this._idleTimer);
      this._idleTimer = null;
    }
    // Keep idleTimeoutMs configuration for future connections
  }
  ...
  async _createSSETransport(authProvider, resolvedConfig) {
    ...
    // SSE transport setup WITHOUT reconnection support
    // Reconnection breaks session IDs since each connection gets a unique session
    // If connection is lost, a new connection with new session ID must be established
    const transport = new SSEClientTransport(new URL(resolvedConfig.url), {
      requestInit: {
        headers: resolvedConfig.headers, // Already resolved with commands support
      },
      authProvider,
      // No eventSourceInit or custom EventSource - use standard EventSource
      // This prevents automatic reconnection that would break session continuity
    });
    
    // Add connection monitoring without automatic reconnection
    if (transport.eventSource) {
      transport.eventSource.addEventListener('error', (event) => {
        logger.debug(`SSE connection error for '${this.name}' at ${resolvedConfig.url}`);
        // Don't reconnect automatically - let the connection fail and require manual reconnection
      });
      
      transport.eventSource.addEventListener('open', (event) => {
        logger.debug(`SSE connection opened for '${this.name}' at ${resolvedConfig.url}`);
      });
    }
  ...
  async _handleDevFilesChanged(data) {
    try {
