...
class Logger {
  ...
  log(type, message, data = {}, code = null, options = {}) {
    const { exit = false, exitCode = 1, level = type, skipSSEBroadcast = false } = options;
    ...
    // Skip SSE broadcast if explicitly requested (to prevent recursion)
    if (this.sseManager && this.LOG_LEVELS[level] <= this.LOG_LEVELS[this.logLevel] && !skipSSEBroadcast) {
      try {
        this.sseManager.broadcast('log', entry);
      } catch (broadcastError) {
        // Prevent recursion: log SSE broadcast errors but don't broadcast them
        console.error(JSON.stringify({
          type: 'error',
          message: 'SSE broadcast failed',
          data: { originalMessage: message, broadcastError: broadcastError.message },
          timestamp: new Date().toISOString(),
          code: 'SSE_BROADCAST_RECURSION_GUARD'
        }));
      }
    }
...
process.on("uncaughtException", (error) => {
  logger.log('error', `An unhandled error occurred: ${error}`, 
    { message: error.message, stack: error.stack },
    error.code || "UNHANDLED_ERROR",
    { skipSSEBroadcast: true }
  );
});

// Handle unhandled promise rejections 
process.on("unhandledRejection", (error) => {
  logger.log('error', `An unhandled rejection occurred: ${error}`,
    { message: error?.message, stack: error?.stack },
    "UNHANDLED_REJECTION",
    { skipSSEBroadcast: true }
  );
