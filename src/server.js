...
class ServiceManager {
  ...
  async initializeMCPHub() {
    ...
    try {
      ...
      logger.log('error', "Failed to initialize MCP server endpoint", {
        error: error.message
      }, "MCP_ENDPOINT_INIT_ERROR", { exit: false, skipSSEBroadcast: true });
    }
  ...
  async stopServer() {
    return new Promise((resolve, reject) => {
      ...
      this.server.close((error) => {
        if (error) {
        logger.log('error', "Failed to stop HTTP server", {
          error: error.message,
          stack: error.stack,
        }, "SERVER_STOP_ERROR", { exit: false, skipSSEBroadcast: true });
  ...
  async stopMCPHub() {
    ...
    try {
      ...
      logger.log('error', "Failed to stop MCP Hub", {
        error: error.message,
        stack: error.stack,
      }, "HUB_STOP_ERROR", { exit: false, skipSSEBroadcast: true });
    }
  }
  ...
  setupSignalHandlers() {
    ...
    process.on('uncaughtException', (error) => {
      logger.log('error', 'Uncaught exception caught - continuing', {
        error: error.message,
        stack: error.stack
      }, 'UNCAUGHT_EXCEPTION', { exit: false, skipSSEBroadcast: true });
    });

    process.on('unhandledRejection', (reason, promise) => {
      logger.log('error', 'Unhandled promise rejection caught - continuing', {
        reason: reason?.message || reason,
        stack: reason?.stack
      }, 'UNHANDLED_REJECTION', { exit: false, skipSSEBroadcast: true });
    });

    // Track shutdown in progress to prevent duplicate shutdowns
    let shutdownInProgress = false;
    
    const shutdown = (signal) => async () => {
      // Prevent duplicate shutdowns
      if (shutdownInProgress) {
        logger.log('debug', `Ignoring duplicate ${signal} signal - shutdown already in progress`, {
          signal
      ...
      try {
        ...
        logger.log('error', "Shutdown failed", {
          error: error.message,
          stack: error.stack,
        }, "SHUTDOWN_ERROR", { exit: true, exitCode: 1, skipSSEBroadcast: true });
      }
    };
...
registerRoute("GET", "/events", "Subscribe to server events", async (req, res) => {
  try {
    ...
    logger.log('error', 'Failed to setup SSE connection', {
      error: error.message,
      stack: error.stack
    }, 'SSE_SETUP_ERROR', { skipSSEBroadcast: true });
...
app.get("/mcp", async (req, res) => {
  try {
    if (!mcpServerEndpoint) {
      logger.error("MCP_ENDPOINT_ERROR", "MCP server endpoint not initialized", {}, false);
      return res.status(503).json({
        error: "MCP server not ready",
        message: "The MCP server endpoint is not initialized yet. Please try again in a few seconds.",
        retry_after: 5
      });
    }
    
    if (!serviceManager?.isReady()) {
      logger.debug("MCP connection attempted before hub is ready");
      return res.status(503).json({
        error: "Hub not ready",
        message: "MCP Hub is still starting up. Please try again in a few seconds.",
        state: serviceManager?.state || "unknown",
        retry_after: 3
      });
    }
    ...
    if (!res.headersSent) {
      res.status(500).json({
        ...
        message: "Failed to establish MCP connection. Please try again.",
...
// =============================================================================
// CENTRALIZED TOOL INDEX API ENDPOINTS
// =============================================================================

// Register tool index endpoints
registerRoute(
  "POST",
  "/tools/register",
  "Register a server and its tools with the centralized index",
  async (req, res) => {
    const { name, endpoint, tools = [], capabilities = {}, metadata = {} } = req.body;
    
    try {
      if (!name) {
        throw new ValidationError("Missing server name", { field: "name" });
      }
      if (!endpoint) {
        throw new ValidationError("Missing server endpoint", { field: "endpoint" });
      }

      const result = await toolIndex.registerServer({
        name,
        endpoint,
        tools,
        capabilities,
      ...
      throw wrapError(error, "TOOL_REGISTRATION_ERROR", {
        serverName: name,
...
registerRoute(
  "DELETE",
  "/tools/unregister/:serverName",
  "Unregister a server and its tools from the index",
  async (req, res) => {
    const { serverName } = req.params;
    
    try {
      const success = await toolIndex.unregisterServer(serverName);
      
      if (!success) {
        throw new ValidationError("Server not found in index", { serverName });
      }
      ...
      throw wrapError(error, "TOOL_UNREGISTRATION_ERROR", { serverName });
    }
  }
);
...
registerRoute(
  "GET",
  "/tools/list",
  "List all tools in the centralized index with optional filtering",
  async (req, res) => {
    const {
      server,
      category,
    ...
    try {
      ...
      throw wrapError(error, "TOOL_LIST_ERROR", { query: req.query });
    }
  }
);
...
registerRoute(
  ...
  async (req, res) => {
    ...
    try {
      if (!pattern) {
        throw new ValidationError("Missing search pattern", { field: "q" });
      }

      const result = toolIndex.findTools(pattern, {
        searchIn,
      ...
      throw wrapError(error, "TOOL_SEARCH_ERROR", {
        pattern,
        options: req.query
      });
    }
...
registerRoute(
  ...
  async (req, res) => {
    ...
    try {
      const tool = toolIndex.getTool(toolId);
      ...
      throw wrapError(error, "TOOL_GET_ERROR", { toolId });
    }
  }
);
...
registerRoute(
  "GET",
  "/tools/:toolId/routing",
  "Get routing information for executing a specific tool",
  async (req, res) => {
    const { toolId } = req.params;
    
    try {
      const routing = toolIndex.getToolRouting(toolId);
      ...
      throw wrapError(error, "TOOL_ROUTING_ERROR", { toolId });
    }
  }
);
...
registerRoute(
  ...
  "Execute a tool through the centralized index routing",
  async (req, res) => {
    const { toolId } = req.params;
    ...
    try {
      ...
      // Record usage
      toolIndex.recordToolUsage(toolId);
      
      // Execute tool via MCP Hub
      const result = await serviceManager.mcpHub.callTool(
        routing.serverName,
        routing.originalName,
        args
      );
...
registerRoute(
  "GET",
  "/tools/index/stats",
  "Get statistics about the centralized tool index",
  async (req, res) => {
    try {
      ...
      throw wrapError(error, "TOOL_STATS_ERROR");
    }
  }
);
...
registerRoute(
  "DELETE",
  "/tools/index/clear",
  "Clear the entire tool index (admin only)",
  async (req, res) => {
    try {
      toolIndex.clear();
      ...
      res.json({
        status: "ok",
        message: "Tool index cleared",
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      throw wrapError(error, "TOOL_INDEX_CLEAR_ERROR");
    }
  }
);

// =============================================================================
// END TOOL INDEX API ENDPOINTS
// =============================================================================
...
registerRoute("GET", "/health", "Check server health", async (req, res) => {
  ...
  // Add server loading manager status if available
  if (serviceManager?.mcpHub?.serverLoadingManager) {
    try {
      healthData.serverLoadingManager = serviceManager.mcpHub.serverLoadingManager.getStatus();
    } catch (error) {
      logger.debug(`Failed to get server loading manager status: ${error.message}`);
    }
  }
...
// =============================================================================
// SERVER LOADING MANAGER API ENDPOINTS
// =============================================================================

// Get server loading manager status
registerRoute(
  "GET",
  "/servers/loading-status",
  "Get server loading manager status and statistics",
  async (req, res) => {
    try {
      if (!serviceManager.mcpHub.serverLoadingManager) {
        throw new ServerError("Server loading manager not available");
      }
      
      const status = serviceManager.mcpHub.serverLoadingManager.getStatus();
      res.json({
        status: "ok",
        loadingManager: status,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
      throw wrapError(error, "LOADING_MANAGER_STATUS_ERROR");
    }
  }
);

// Trigger manual server loading
registerRoute(
  "POST",
  "/servers/trigger-load",
  "Manually trigger loading of a specific server",
  async (req, res) => {
    const { server_name } = req.body;
    try {
      if (!server_name) {
      ...
      await serviceManager.mcpHub.serverLoadingManager.triggerServerLoad(server_name);
      const status = serviceManager.mcpHub.getServerStatus(server_name);
      
      res.json({
        status: "ok",
        server: status,
        message: `Server '${server_name}' loading triggered`,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
      throw wrapError(error, "MANUAL_LOAD_ERROR", { server: server_name });
    }
  }
);

// Get persistent tool registry
registerRoute(
  "GET",
  "/servers/persistent-registry",
  "Get persistent tool registry information",
  async (req, res) => {
    try {
      if (!serviceManager.mcpHub.serverLoadingManager) {
        throw new ServerError("Server loading manager not available");
      ...
      const registry = serviceManager.mcpHub.serverLoadingManager.persistentRegistry;
      const tools = Array.from(registry.tools.entries()).map(([id, tool]) => ({
        id,
        ...tool
      }));
      const servers = Array.from(registry.servers.entries()).map(([name, server]) => ({
        name,
        ...server
      }));
      ...
      throw wrapError(error, "PERSISTENT_REGISTRY_ERROR");
    }
  }
);
...
registerRoute(
  ...
  async (req, res) => {
    ...
    try {
      ...
      logger.log('error', `Error during OAuth callback: ${error.message}`, {}, 'OAUTH_CALLBACK_ERROR', { exit: false, skipSSEBroadcast: true })
...
export async function startServer(options = {}) {
  ...
  try {
    ...
    // Then initialize MCP Hub with error handling
    try {
      await serviceManager.initializeMCPHub();
      logger.info("MCP Hub startup completed successfully - server running");
    } catch (hubError) {
      logger.log('error', "MCP Hub initialization failed, but HTTP server continues running", {
        error: hubError.message,
        stack: hubError.stack
      }, "MCP_HUB_INIT_ERROR", { exit: false, skipSSEBroadcast: true });
      // Don't exit - keep HTTP server running even if MCP Hub fails
    }
  } catch (error) {
    ...
    try {
      ...
      logger.log('error', wrappedError.message, wrappedError.data, wrappedError.code, { exit: true, exitCode: 1, skipSSEBroadcast: true });
    }
  }
}
