  ...
  describe("Server Management", () => {
    it("should start enabled servers from config", async () => {
      ...
      expect(MCPConnection).toHaveBeenCalledWith(
        ...
        "http://localhost:undefined"  // hubServerUrl
      );
    ...
    it("should skip disabled servers", async () => {
      ...
      // With the new ServerLoadingManager, disabled servers are handled differently
      // They are logged with debug level during initialization, not as "Skipping disabled server"
      expect(logger.debug).toHaveBeenCalledWith("Skipping disabled MCP server", {
        server: "server2",
      });
    });
    ...
    it("should disconnect server", async () => {
      ...
      // Connection should be kept for lazy loading, not removed
    ...
    it("should handle disconnect errors", async () => {
      ...
      // Connection should be kept for lazy loading even when disconnect errors occur
      expect(mcpHub.connections.has("server1")).toBe(true);
    });
