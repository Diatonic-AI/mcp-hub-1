...
describe("CLI", () => {
  ...
  it("should handle server start errors", async () => {
    ...
    // With new error handling, non-fatal errors don't send SIGINT
    expect(mockKill).not.toHaveBeenCalled();
  });

  it("should handle fatal errors", async () => {
    const fatalError = new Error("EADDRINUSE: port already in use");
    server.startServer.mockRejectedValueOnce(fatalError);

    setArgv(["--port", "3000", "--config", "./config.json"]);
    await import("../src/utils/cli.js");

    // Fatal errors (port conflicts) should exit
    expect(mockExit).toHaveBeenCalledWith(1);
  });
});
